...
public class PhoneDirectory {
	 ...
	 * look for both in different threads
	 */
	public ArrayList<PhoneDirectoryEntry> search(String name, String number) {
		String input = "";
		...
		ArrayList<PhoneDirectoryEntry> result = new ArrayList<PhoneDirectoryEntry>();

		if (!input.matches("\\s*") || input.length() != 0) {
			
			// if both is entered
			if (name != null && number != null) {
				ThreadedSearch<PhoneDirectoryEntry>[] erg = this.searchForBoth(name, number);
				result.addAll(erg[0].getResults().getPhonelist());
				result.addAll(erg[1].getResults().getPhonelist());
			}
			// if name is only entered
			else if (name != null) {
				ThreadedSearch<PhoneDirectoryEntry>[] erg = this.searchForName(name);
				result.addAll(erg[0].getResults().getPhonelist());
			}					
			// if number is only entered
			else if (number != null) {
				ThreadedSearch<PhoneDirectoryEntry>[] erg = this.searchForNumber(number.trim());
				result.addAll(erg[0].getResults().getPhonelist());
			}
